...
def busca_profundidade_algoritmo(matriz_adj: list, n_nos: int, no_comeco: int, branco: list,
                                                        preto: list, cinza: list, raizes: list):
    ...
    for no_atual in matriz_adj[no_comeco_pos]:
        if no_atual != 0:   # verifica se existe aresta entre os nos
            # se em algum momento um no for encontrado percorrendo a arvore, entao ele nao pode ser raiz
            if no_atual in raizes:
                raizes.remove(no_atual)

            if no_atual in branco:
                raizes, preto = busca_profundidade_algoritmo(matriz_adj, n_nos, no_atual, branco, preto, cinza, raizes)
...
def busca_profundidade(matriz_adj: list, n_nos: int, id_nos):
    ...
    while branco: # enquanto ainda existirem elementos para serem iterados
        raizes, preto = busca_profundidade_algoritmo(matriz_adj, n_nos, branco[0], branco, preto, cinza, raizes) # +1 pra ser no e nao pos
        for no in preto:
            if no in branco:
                branco.remove(no)
    return raizes
...
def achar_fontes(matriz_adj: list, n_nos: list, id_nos):
    raizes = busca_profundidade(matriz_adj, n_nos, id_nos)
    print(len(raizes))
